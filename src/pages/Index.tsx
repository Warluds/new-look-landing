...
import logoBurgerKing from "@/assets/partners/burger-king.png";
import logoWendys from "@/assets/partners/wendys.png";
import logoCoffeeBoom from "@/assets/partners/coffee-boom.png";
import logoLaTartine from "@/assets/partners/la-tartine.png";
import logoShafran from "@/assets/partners/shafran.png";
import logoLorena from "@/assets/partners/lorena-antoniazzi.png";
import logoInvictus from "@/assets/partners/invictus-go.png";
import logoCompass from "@/assets/partners/compass.png";
import logoKustoHome from "@/assets/partners/kusto-home.png";
...
const reviews: Array<{ name: string; text: Tr; source: string; href: string }> = [
  ...
  ...([
    ["Aisha A", "Хочу выразить огромную благодарность Тамаре за помощь в выборе люстр! Очень приятный, вежливый и внимательный человек. Качество отличное, покупкой осталась очень довольна.", "S"],
    ["Samal Aitkalieva", "Отличный магазин! Большое спасибо менеджеру Болату за профессиональную консультацию, внимательное отношение и помощь в выборе. Всё подробно объяснил и ответил на все вопросы.", "K"],
    ["Nurshat Ibibullaev", "Отличный магазин освещения! Помогли быстро подобрать подходящие светильники, всё качественно и красиво. Спасибо Камиле за доброжелательность и профессиональный подход!", "S"],
    ["Никош T", "Всё понравилось: выбор красок, обслуживание и отношение персонала. Отдельное спасибо Нурболу за консультацию. Кофе тоже отличный.", "K"],
    ["Sezim", "Покупали люстру и бра в спальню. Очень понравилось, что помогли с выбором и всё подробно объяснили. Светильники пришли качественные, без дефектов.", "S"],
    ["Nazyma K", "Очень люблю этот магазин. Краски очень хорошего качества, также всегда там приобретаем валики и кисточки. Спасибо менеджеру Рамису за профессионализм. Всем рекомендую 🌸", "K"],
    ["n k", "Хороший салон с большим выбором освещения на любой вкус. Персонал вежливый и отзывчивый. Благодарю Камилу за помощь в подборе светильников и отличный сервис.", "S"],
    ["Ramazan Matsey", "Очень вежливый, внимательный и грамотный консультант Болат. Помог с выбором, всё подробно объяснил, остался доволен обслуживанием. Обязательно вернусь снова.", "K"],
    ["Parhat Azakhunov", "Покупали тут люстры, всё быстро оперативно подобрали и сделали доставку, остались очень довольны. Спасибо менеджеру Малике, очень ответственная.", "S"],
    ["Турсын Галымбеков", "Хороший ассортимент и приятное обслуживание. Спасибо Азизу за консультацию, подсказал по выбору краски. Кофе тоже отличный.", "K"],
    ["Zhanibek Salim", "Хотел бы поблагодарить менеджера Камиллу! Всё подробно и грамотно подсказала. Выбрать люстру было очень легко и просто!", "S"],
    ["Saltanat Umbitalina", "Покупаю в Центре красок уже давно, всегда обслуживание быстрое и качественное 👍 Онлайн всё налажено.", "K"],
    ["Тима Узакбай", "Обращались за подбором трековой системы для кухни. Всё рассчитали, подсказали. Маликаға рахмет! ❤️", "S"],
    ["Moldir Ishtaeva", "Атмосфера нравится, добрые отзывчивые сотрудники. Болат помог подобрать цвет для моего интерьера — огромная благодарность!", "K"],
    ["Galina Velichko", "Спасибо салону за хорошую работу и за качественную продукцию, всегда рады вернуться и приобрести ещё 😊", "S"],
    ["Наурызбай Айтиев", "Понравился подход консультанта. Давир не просто показал товар, а помог разобраться с выбором.", "K"],
  ] as const).map(([name, text, s]) => ({
    name,
    text: [text, text, text] as Tr,
    source: s === "S" ? "SVET.KZ" : "Центр Красок №1",
    href: s === "S" ? SVET_2GIS : KRASKI_2GIS,
  })),
];
